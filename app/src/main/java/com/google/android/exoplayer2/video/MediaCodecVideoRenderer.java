...
public class MediaCodecVideoRenderer extends MediaCodecRenderer {
  ...
  public static final class VideoDecoderException extends DecoderException {
    ...
    public VideoDecoderException(Throwable cause, @Nullable MediaCodecInfo codecInfo, @Nullable Surface surface) {
  ...
  public MediaCodecVideoRenderer(Context context, MediaCodecSelector mediaCodecSelector, long allowedJoiningTimeMs) {
    this(context,mediaCodecSelector,allowedJoiningTimeMs,
        /* eventHandler= */ null,
        /* eventListener= */ null,
  ...
  @Deprecated
  @SuppressWarnings("deprecation")
  public MediaCodecVideoRenderer(Context context, MediaCodecSelector mediaCodecSelector, long allowedJoiningTimeMs, DrmSessionManager<FrameworkMediaCrypto> drmSessionManager,
      boolean playClearSamplesWithoutKeys, Handler eventHandler, VideoRendererEventListener eventListener, int maxDroppedFramesToNotify) {
    this(context,mediaCodecSelector,allowedJoiningTimeMs,drmSessionManager,playClearSamplesWithoutKeys, /* enableDecoderFallback= */ false, eventHandler,eventListener,maxDroppedFramesToNotify);
  }
  ...
  @Deprecated
  public MediaCodecVideoRenderer(Context context,MediaCodecSelector mediaCodecSelector,long allowedJoiningTimeMs, DrmSessionManager<FrameworkMediaCrypto> drmSessionManager,
      boolean playClearSamplesWithoutKeys,boolean enableDecoderFallback, Handler eventHandler, VideoRendererEventListener eventListener, int maxDroppedFramesToNotify) {

    super(C.TRACK_TYPE_VIDEO, mediaCodecSelector, drmSessionManager, playClearSamplesWithoutKeys, enableDecoderFallback,/* assumedMinimumCodecOperatingRate= */ 30);
  ...
  @Override
  @Capabilities
  protected int supportsFormat(MediaCodecSelector mediaCodecSelector, @Nullable DrmSessionManager<FrameworkMediaCrypto> drmSessionManager, Format format) throws DecoderQueryException {
    ...
    if (requiresSecureDecryption && decoderInfos.isEmpty()) {
      ...
      decoderInfos = getDecoderInfos(mediaCodecSelector, format,  /* requiresSecureDecoder= */ false, /* requiresTunnelingDecoder= */ false);
    }
    ...
    if (isFormatSupported) {
      List<MediaCodecInfo> tunnelingDecoderInfos = getDecoderInfos(mediaCodecSelector, format, requiresSecureDecryption, /* requiresTunnelingDecoder= */ true);
  ...
  private static List<MediaCodecInfo> getDecoderInfos(MediaCodecSelector mediaCodecSelector, Format format, boolean requiresSecureDecoder, boolean requiresTunnelingDecoder) throws DecoderQueryException {
    ...
    if (MimeTypes.VIDEO_DOLBY_VISION.equals(mimeType)) {
      ...
      if (codecProfileAndLevel != null) {
        ...
        if (profile == CodecProfileLevel.DolbyVisionProfileDvheDtr || profile == CodecProfileLevel.DolbyVisionProfileDvheSt) {
...
//    }
  ...
  protected int canKeepCodec(MediaCodec codec, MediaCodecInfo codecInfo, Format oldFormat, Format newFormat) {
    if (codecInfo.isSeamlessAdaptationSupported(oldFormat, newFormat, /* isNewFormatComplete= */ true)
        && newFormat.width <= codecMaxValues.width   && newFormat.height <= codecMaxValues.height && getMaxInputSize(codecInfo, newFormat) <= codecMaxValues.inputSize) {
  ...
  protected void onCodecInitialized(String name, long initializedTimestampMs, long initializationDurationMs) {
    ...
    codecHandlesHdr10PlusOutOfBandMetadata =  Assertions.checkNotNull(getCodecInfo()).isHdr10PlusOutOfBandMetadataSupported();
  }
  ...
  protected void onOutputFormatChanged(MediaCodec codec, MediaFormat outputMediaFormat) {
    ...
    boolean hasCrop = outputMediaFormat.containsKey(KEY_CROP_RIGHT)
    ...
    int width = hasCrop ? outputMediaFormat.getInteger(KEY_CROP_RIGHT) - outputMediaFormat.getInteger(KEY_CROP_LEFT) + 1 : outputMediaFormat.getInteger(MediaFormat.KEY_WIDTH);
    int height =  hasCrop  ? outputMediaFormat.getInteger(KEY_CROP_BOTTOM)  - outputMediaFormat.getInteger(KEY_CROP_TOP) + 1 : outputMediaFormat.getInteger(MediaFormat.KEY_HEIGHT);
  ...
  @Override
  protected boolean processOutputBuffer(long positionUs, long elapsedRealtimeUs, MediaCodec codec, ByteBuffer buffer, int bufferIndex, int bufferFlags,
      long bufferPresentationTimeUs, boolean isDecodeOnlyBuffer, boolean isLastBuffer, Format format, int outputBufferInfoSize) throws ExoPlaybackException {
...
//    }
    ...
    // 是否强制输出第一帧图像，一般是在准备一个视频文件，加载完成后不需要立即播放的情况。
    boolean forceRenderOutputBuffer =  joiningDeadlineMs == C.TIME_UNSET && positionUs >= outputStreamOffsetUs && (!renderedFirstFrame  || (isStarted && shouldForceRenderOutputBuffer(earlyUs, elapsedSinceLastRenderUs)));
    if (forceRenderOutputBuffer) {
      long releaseTimeNs = System.nanoTime();
      notifyFrameMetadataListener(presentationTimeUs, releaseTimeNs, format, currentMediaFormat);
      if (Util.SDK_INT >= 21) {
        renderOutputBufferV21(codec, bufferIndex, presentationTimeUs, releaseTimeNs, buffer, outputBufferInfoSize);
      } else {
        renderOutputBuffer(codec, bufferIndex, presentationTimeUs, buffer, outputBufferInfoSize);
      }
      return true;
    }
    ...
    if (shouldDropBuffersToKeyframe(earlyUs, elapsedRealtimeUs, isLastBuffer) && maybeDropBuffersToKeyframe( codec, bufferIndex, presentationTimeUs, positionUs, treatDroppedBuffersAsSkipped)) {
      return false;
    ...
    if (Util.SDK_INT >= 21) {
      ...
      if (earlyUs < 50000) {
        ...
        renderOutputBufferV21(codec, bufferIndex, presentationTimeUs, adjustedReleaseTimeNs, buffer, outputBufferInfoSize);
        return true;
      }
    } else {
  ...
  private void notifyFrameMetadataListener(long presentationTimeUs, long releaseTimeNs, Format format, MediaFormat mediaFormat) {
    if (frameMetadataListener != null) {
      frameMetadataListener.onVideoFrameAboutToBeRendered(presentationTimeUs, releaseTimeNs, format, mediaFormat);
    }
  }
  ...
  protected void onProcessedOutputBuffer(long presentationTimeUs) {
    ...
    while (pendingOutputStreamOffsetCount != 0  && presentationTimeUs >= pendingOutputStreamSwitchTimesUs[0]) {
      ...
      System.arraycopy(pendingOutputStreamOffsetsUs, /* srcPos= */ 1, pendingOutputStreamOffsetsUs, /* destPos= */ 0, pendingOutputStreamOffsetCount);

      System.arraycopy(pendingOutputStreamSwitchTimesUs, /* srcPos= */ 1, pendingOutputStreamSwitchTimesUs, /* destPos= */ 0, pendingOutputStreamOffsetCount);
      clearRenderedFirstFrame();
    }
  }
  ...
  private static boolean isBufferLate(long earlyUs) {
    ...
    //如果一个缓冲区应该在30毫秒之前出现，那么它应该延迟一段时间。
    return earlyUs < -30000;
  }

  private static boolean isBufferVeryLate(long earlyUs) {
    // Class a buffer as very late if it should have been presented more than 500 ms ago.
    // 如果一个缓冲区在500毫秒前就应该出现的话，那么它会很晚出现。
  ...
  protected CodecMaxValues getCodecMaxValues(MediaCodecInfo codecInfo, Format format, Format[] streamFormats) {
    ...
    if (streamFormats.length == 1) {
      ...
      // being configured.
      // streamFormats中的单个条目必须与配置编解码器的格式相对应。
    ...
    for (Format streamFormat : streamFormats) {
      if (codecInfo.isSeamlessAdaptationSupported(format, streamFormat, /* isNewFormatComplete= */ false)) {
  ...
  private static int getMaxInputSize(MediaCodecInfo codecInfo, Format format) {
    if (format.maxInputSize != Format.NO_VALUE) {
      ...
      // 格式定义了显式的最大输入大小。添加初始化数据缓冲区的总大小，因为它们可能需要在与最大样本相同的输入缓冲区中排队。
    ...
    switch (sampleMimeType) {
      ...
      case MimeTypes.VIDEO_H264:
        if ("BRAVIA 4K 2015".equals(Util.MODEL) // Sony Bravia 4K
            || ("Amazon".equals(Util.MANUFACTURER)  && ("KFSOWI".equals(Util.MODEL) // Kindle Soho
          ...
          // 对于平台限制可能阻止分配计算出的最大输入大小的缓冲区的情况，请使用默认值。
          return Format.NO_VALUE;
        }
      ...
      case MimeTypes.VIDEO_VP8:
        ...
        // VPX没有指定比率，因此使用平台的SoftVPX.cpp软件.
        maxPixels = width * height;
        minCompressionRatio = 2;
        break;
   ...
   * ExoPlayer.
   * 返回默认情况下设备是否执行与ExoPlayer不兼容的后处理
   *
   * @return Whether the device is known to do post processing by default that isn't compatible with ExoPlayer.
   * 是否知道设备在默认情况下执行与ExoPlayer不兼容的后期处理。
   */
  ...
  private final class OnFrameRenderedListenerV23 implements MediaCodec.OnFrameRenderedListener, Handler.Callback {
    ...
    public void onFrameRendered(MediaCodec codec, long presentationTimeUs, long nanoTime) {
      ...
      if (Util.SDK_INT < 30) {
        Message message = Message.obtain(handler, /* what= */ HANDLE_FRAME_RENDERED, /* arg1= */ (int) (presentationTimeUs >> 32), /* arg2= */ (int) presentationTimeUs);
