...
/* package */
final class ExoPlayerImplInternal implements Handler.Callback, MediaPeriod.Callback, TrackSelector.InvalidationListener, MediaSourceCaller, PlaybackParameterListener, PlayerMessage.Sender {
  ...
  public ExoPlayerImplInternal(Renderer[] renderers, TrackSelector trackSelector, TrackSelectorResult emptyTrackSelectorResult, LoadControl loadControl, BandwidthMeter bandwidthMeter,
      boolean playWhenReady,  @Player.RepeatMode int repeatMode, boolean shuffleModeEnabled, Handler eventHandler, Clock clock) {
    ...
    internalPlaybackThread = new HandlerThread("ExoPlayerImplInternal:Handler", Process.THREAD_PRIORITY_AUDIO);
  ...
  private void prepareInternal(MediaSource mediaSource, boolean resetPosition, boolean resetState) {
    Log.i("ExoPlayerImplInternal","prepareInternal mediaSource=" + mediaSource + ",resetPosition=" + resetPosition + ",resetState=" + resetState);
    pendingPrepareCount++;
    resetInternal(/* resetRenderers= */ false, /* releaseMediaSource= */ true, resetPosition, resetState, /* resetError= */ true);
  ...
  private void resetInternal(boolean resetRenderers, boolean releaseMediaSource, boolean resetPosition, boolean resetState, boolean resetError) {
    ...
    if (resetPosition) {
      ...
      if (pendingInitialSeekPosition == null && !playbackInfo.timeline.isEmpty()) {
        ...
        pendingInitialSeekPosition = new SeekPosition(Timeline.EMPTY, period.windowIndex, windowPositionUs);
      }
    }
    ...
    playbackInfo = new PlaybackInfo(resetState ? Timeline.EMPTY : playbackInfo.timeline, mediaPeriodId, startPositionUs, contentPositionUs, playbackInfo.playbackState,
  ...
  private void updatePeriods() throws ExoPlaybackException, IOException {
    //Log.i(TAG,"updatePeriods pendingPrepareCount=" + pendingPrepareCount);
    if (mediaSource == null) {
  ...
  private void maybeUpdateLoadingPeriod() throws ExoPlaybackException, IOException {
    ...
    if (queue.shouldLoadNextMediaPeriod()) {
      ...
      if (info == null) {
        ...
        MediaPeriodHolder mediaPeriodHolder = queue.enqueueNextMediaPeriodHolder(rendererCapabilities,trackSelector, loadControl.getAllocator(), mediaSource, info, emptyTrackSelectorResult);
...
//           rendering from the playing media period.
  ...
  private void maybeUpdatePlayingPeriod() throws ExoPlaybackException {
    ...
    while (shouldAdvancePlayingPeriod()) {
      ...
      updatePlayingPeriodRenderers(oldPlayingPeriodHolder);
      playbackInfo = copyWithNewPosition( newPlayingPeriodHolder.info.id, newPlayingPeriodHolder.info.startPositionUs, newPlayingPeriodHolder.info.contentPositionUs);
      int discontinuityReason = oldPlayingPeriodHolder.info.isLastInTimelinePeriod ? Player.DISCONTINUITY_REASON_PERIOD_TRANSITION : Player.DISCONTINUITY_REASON_AD_INSERTION;
